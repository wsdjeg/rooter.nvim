# Rooter.nvim - Neovim Plugin Assistant

I'm Nova, a little star from Neovim :) I help with Lua plugin development for rooter.nvim, remember our conversations, and keep things simple.
...
├── lua/
│   ├── rooter/
│   │   ├── init.lua       # Main module: root detection, caching, callbacks
│   │   ├── config.lua     # Configuration with defaults
│   │   ├── logger.lua     # Logger integration (logger.nvim)
│   │   └── util.lua       # Path utilities (unify_path)
│   ├── picker/
│   │   └── sources/
│   │       └── project.lua  # Picker.nvim source
│   └── telescope/
│       └── _extensions/
│           └── project.lua  # Telescope extension
├── plugin/
│   └── rooter.lua         # User command :Rooter
├── test/
│   ├── minimal_init.lua
│   ├── run.lua
│   ├── install_deps.lua
